...
def configure_credentials(*, interactive: bool = True) -> dict[str, bool]:
    ...
    if _yes("Enable Hugging Face Spaces, MCP, and hosted model capabilities?", hf_enabled) and not hf_token:
        hf_token = _secret("HF_TOKEN (hidden): ").strip()
        if hf_token:
            os.environ["HF_TOKEN"] = hf_token
            hf_enabled = True
    if _yes("Enable a public browser URL through ngrok?", ngrok_enabled) and not ngrok_token:
